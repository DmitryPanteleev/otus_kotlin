...
import ru.otus.otuskotlin.marketplace.core.handlers.CoreChain
import ru.otus.otuskotlin.marketplace.core.handlers.CoreWorker
import ru.otus.otuskotlin.marketplace.core.handlers.executeSequential
...
class CorBaseTest {
	...
	fun `worker should execute handle`() = runTest {
		val worker = CoreWorker<TestContext>(
			title = "w1",
	...
	private suspend fun execute(dsl: ICoreExecDsl<TestContext>): TestContext {
		val ctx = TestContext()
		dsl.build().exec(ctx)
		return ctx
	}
...
private fun ICoreChainDsl<TestContext>.printResult() = worker(title = "Print example") {
	println("some = $some")
}
