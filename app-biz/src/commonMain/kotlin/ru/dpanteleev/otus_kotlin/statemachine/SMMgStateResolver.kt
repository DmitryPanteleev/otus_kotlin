...
class SMMgStateResolver {
    ...
    companion object {
        ...
        private val TR_MX = mapOf(
            Sig(SMStates.NEW, SMDurs.D_NEW, SMViews.FEW) to SMTransition(SMStates.NEW, "Новое без изменений"),
            Sig(SMStates.NEW, SMDurs.D_ACT, SMViews.FEW) to SMTransition(SMStates.ACTUAL, "Вышло время, перевод из нового в актуальное"),
            Sig(SMStates.NEW, SMDurs.D_NEW, SMViews.MODER) to SMTransition(
                SMStates.HIT,
                "Много просмотров, стало хитом"
            ),
            Sig(SMStates.NEW, SMDurs.D_NEW, SMViews.LARGE) to SMTransition(
                SMStates.HIT,
                "Очень много просмотров, стало хитом"
            ),
            Sig(SMStates.HIT, SMDurs.D_NEW, SMViews.MODER) to SMTransition(SMStates.HIT, "Остается хитом"),
            Sig(SMStates.HIT, SMDurs.D_ACT, SMViews.MODER) to SMTransition(
                SMStates.ACTUAL,
                "Время вышло, хит утих, становится актуальным"
            ),
            Sig(SMStates.HIT, SMDurs.D_ACT, SMViews.LARGE) to SMTransition(
                SMStates.ACTUAL,
                "Время вышло, хит становится популярным"
            ),
            Sig(SMStates.NEW, SMDurs.D_OLD, SMViews.FEW) to SMTransition(
                SMStates.OLD,
                "Устарело, просмотров мало, непопулярное и старое объявление"
            ),
        )
        private val TR_ERROR = SMTransition(SMStates.ERROR, "Unprovided transition occurred")
    }
}
