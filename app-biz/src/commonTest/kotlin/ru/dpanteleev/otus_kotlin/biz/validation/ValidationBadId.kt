...
fun validationIdCorrect(command: Command, processor: MgProcessor) = runTest {
	val ctx = Context(
		command = command,
		state = State.NONE,
...
@OptIn(ExperimentalCoroutinesApi::class)
fun validationIdTrim(command: Command, processor: MgProcessor) = runTest {
	val mortgageRequest = Stub.get()
	mortgageRequest.id = MortgageId("666")
	val ctx = Context(
		command = command,
		state = State.NONE,
		workMode = WorkMode.TEST,
...
@OptIn(ExperimentalCoroutinesApi::class)
fun validationIdEmpty(command: Command, processor: MgProcessor) = runTest {
	val mortgageRequest = Stub.get()
	mortgageRequest.id = MortgageId("")
	val ctx = Context(
		command = command,
		state = State.NONE,
		workMode = WorkMode.TEST,
...
@OptIn(ExperimentalCoroutinesApi::class)
fun validationIdFormat(command: Command, processor: MgProcessor) = runTest {
	val mortgageRequest = Stub.get()
	mortgageRequest.id = MortgageId("!@#\$%^&*(),.{}")
	val ctx = Context(
		command = command,
